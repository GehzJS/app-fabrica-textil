        ...
        <v-data-table fixed-header hide-default-footer :headers="titulos" :items="adquisiciones" :items-per-page="adquisiciones.registros" :options.sync="paginacion" :loading="cargandoTabla" height="60vh">
            <template v-slot:top>
                <v-toolbar flat>
                    ...
                    <v-spacer></v-spacer>
                    <v-text-field flat rounded solo-inverted hide-details label="Buscar por proveedor" prepend-inner-icon="search" v-model="busqueda" @keyup="buscarAdquisicion"></v-text-field>
                    <v-spacer></v-spacer>
                        ...
                        <v-card>
                            <v-container>
                                <v-tabs grow color="info">
                                    ...
                                    <v-tab-item>
                                        <v-radio-group v-model="estado" @change="listarAdquisiciones">
                    ...
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <v-btn icon v-on="on" color="info" @click="modalGuardarAdquisicion({estado: true, local: true})">
...
export default {
    ...
    data() {
        return {
            ...
            // campo: 'nombre',
            // estado: 'todas',
            // campos: [{nombre: 'Nombre', clave: 'nombre'}, {nombre: 'Apellido paterno', clave: 'apellido_p'}, {nombre: 'Apellido materno', clave: 'apellido_m'}, {nombre: 'Correo electrónico', clave: 'correo'}, {nombre: 'Teléfono', clave: 'telefono'}],
            // estados: [{nombre: 'Todas', clave: 'todas'}, {nombre: 'Pendientes', clave: 'pendientes'}, {nombre: 'Pagadas', clave: 'pagadas'}]
        }
    },
    computed: {
    ...
    async created() {
        let campos = await this.listarAdquisiciones();
    }
}
</script>