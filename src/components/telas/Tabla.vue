        ...
        <v-data-table fixed-header hide-default-footer :headers="titulos" :items="telas" :items-per-page="telas.registros" :options.sync="paginacion" :loading="cargandoTabla" height="60vh">
            <template v-slot:top>
                <v-toolbar flat>
                    ...
                    <v-spacer></v-spacer>
                    <v-text-field flat rounded solo-inverted hide-details label="Buscar telas" prepend-inner-icon="search" v-model="busqueda" @keyup="buscarTela"></v-text-field>
                    <v-spacer></v-spacer>
                    ...
                    <v-tooltip bottom>
                        <template v-slot:activator="{ on }">
                            <v-btn icon v-on="on" color="info" @click="modalGuardarTela({estado: true})">
                                <v-icon>note_add</v-icon>
                            </v-btn>
                        </template>
                        <span>Nueva tela</span>
                    </v-tooltip>
                </v-toolbar>
            </template>
